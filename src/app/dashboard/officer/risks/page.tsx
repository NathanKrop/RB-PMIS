...
export default async function OfficerRisksPage() {
  ...
  return <div className="space-y-6"><div><h1 className="text-2xl font-semibold">Risk Register</h1><p className="mt-1 text-sm text-muted-foreground">Track risks, escalation, and mitigation effectiveness</p></div><RiskRegister risks={(risks ?? []) as (Risk & { departments: { name: string } | null })[]} departments={(departments ?? []) as Department[]} editable /></div>;
}
