...
import { BeneficiaryForm } from "../../department/beneficiaries/beneficiary-form";
import { Users, UserCheck, MapPin } from "lucide-react";
import type { Beneficiary } from "@/lib/types";
import { Breadcrumb, BreadcrumbItem } from "@/components/ui/breadcrumb";

export default async function OfficerBeneficiariesPage() {
  const supabase = await createClient();

  const [{ data: beneficiaries }, { data: activities }, { data: departments }] = await Promise.all([
    supabase.from("beneficiaries").select("*, departments(name), activities(description)").order("created_at", { ascending: false }),
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <h1 className="text-2xl font-semibold">Beneficiaries</h1>
          <p className="text-sm text-muted-foreground mt-1">Organisation-wide beneficiary database</p>
        </div>
        <BeneficiaryForm activities={activities ?? []} departments={departments ?? []} showDeptSelect />
      </div>
      ...
      {(!beneficiaries || beneficiaries.length === 0) ? (
        ...
        <div className="space-y-3">
          {(beneficiaries as (Beneficiary & { departments: { name: string } | null; activities: { description: string } | null })[]).map((b) => (
            <Card key={b.id}>
              <CardHeader className="pb-2">
                ...
                <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
                  {b.departments?.name && <span>{b.departments.name}</span>}
              ...
              {(b.feedback || b.testimonial) && (
                <CardContent className="text-sm space-y-2">
                  {b.feedback && <div><p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Feedback</p><p className="mt-0.5">{b.feedback}</p></div>}
                  {b.testimonial && <div><p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Testimonial</p><p className="mt-0.5 italic">&quot;{b.testimonial}&quot;</p></div>}
                </CardContent>
              )}
            </Card>
          ))}
